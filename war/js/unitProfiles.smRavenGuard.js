...
ArmyforgeUnitProfiles.smRavenGuard = {
	armyIds: ['SM_Raven_Guard_NETEA'],
	profiles: {
		...
		landspeeder_storm:{name:'Land Speeder Storm',type:'LV',speed:'35cm',armour:'4+',cc:'6+',ff:'5+',weapons:[{name:'Heavy Bolter',range:'30cm',firepower:'AP5+',notes:[]}],abilities:['Scout','Skimmer','Transport']},
		landspeeder_tempest:{name:'Land Speeder Tempest',type:'LV',speed:'35cm',armour:'4+',cc:'6+',ff:'5+',weapons:[{name:'Assault Cannon',range:'30cm',firepower:'AP5+/AT5+',notes:[]},{name:'Tempest Missile Launcher',range:'45cm',firepower:'AP4+/AT5+',notes:[]},{name:'Tempest Missile Launcher',range:'30cm',firepower:'AA5+',notes:[]}],abilities:['Skimmer']},
		...
		storm_eagle:{name:'Storm Eagle',type:'AC/WE',speed:'Bomber',armour:'4+',cc:'6+',ff:'3+',weapons:[{name:'Twin Heavy Bolter',range:'15cm',firepower:'AP4+/AA5+',notes:['FxF']},{name:'2x Twin Lascannon',range:'30cm',firepower:'AT4+/AA5+',notes:['FxF']},{name:'Vengeance Launcher',range:'30cm',firepower:'2BP',notes:['FxF']}],abilities:['Damage Capacity 1','Planetfall','Thick Rear Armour','Transport','May transport four infantry units without Jump Packs or Mounted . Counts as a Thunderhawk for the purposes of being transported.']},
...
ArmyforgeUnitProfiles.findSmRavenGuardProfileByName = function(displayName, listId) {
	if (!displayName) {
		return null;
	}
	if (listId && !ArmyforgeUnitProfiles.smRavenGuard.armyIds.member(listId)) {
		return null;
	}
	var normalized = ArmyforgeUnitProfiles.normalizeSpaceMarineListName(displayName);
	var key = ArmyforgeUnitProfiles.smRavenGuard.nameToKey[normalized];
	if (!key) {
		return null;
	}
	return ArmyforgeUnitProfiles.smRavenGuard.profiles[key] || null;
};
