...
ArmyforgeUnitProfiles.smWhiteScars = {
	armyIds: ['SM_scars_NETEA'],
	profiles: {
...
ArmyforgeUnitProfiles.findSmWhiteScarsProfileByName = function(displayName, listId) {
	if (!displayName) {
		return null;
	}
	if (listId && !ArmyforgeUnitProfiles.smWhiteScars.armyIds.member(listId)) {
		return null;
	}
	var normalized = ArmyforgeUnitProfiles.normalizeSpaceMarineListName(displayName);
	var key = ArmyforgeUnitProfiles.smWhiteScars.nameToKey[normalized];
	if (!key) {
		return null;
	}
	return ArmyforgeUnitProfiles.smWhiteScars.profiles[key] || null;
};
