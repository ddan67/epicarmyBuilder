...
ArmyforgeUnitProfiles.smSalamanders = {
	armyIds: ['SM_salamanders_NETEA'],
	profiles: {
		...
		predator_incinerator:{name:'Predator Incinerator',type:'AV',speed:'30cm',armour:'4+',cc:'6+',ff:'4+',weapons:[{name:'Autocannon',range:'45cm',firepower:'AP5+/AT6+',notes:[]},{name:'2× Heavy Flamer',range:'15cm',firepower:'AP4+',notes:['IC']},{name:'2× Heavy Flamer',range:'15cm',firepower:'Small Arms',notes:['IC']}],abilities:[]},
		...
		land_raider_redeemer:{name:'Land Raider Redeemer',type:'AV',speed:'25cm',armour:'4+',cc:'5+',ff:'3+',weapons:[{name:'2× Flamestorm Cannon',range:'15cm',firepower:'AP3+',notes:['IC']},{name:'2× Flamestorm Cannon',range:'15cm',firepower:'Small Arms',notes:['IC']},{name:'Twin Assault Cannon',range:'30cm',firepower:'AP4+/AT4+',notes:[]}],abilities:['Reinforced Armour','Thick Rear Armour','Transport']},
...
ArmyforgeUnitProfiles.findSmSalamandersProfileByName = function(displayName, listId) {
	if (!displayName) {
		return null;
	}
	if (listId && !ArmyforgeUnitProfiles.smSalamanders.armyIds.member(listId)) {
		return null;
	}
	var normalized = ArmyforgeUnitProfiles.normalizeSpaceMarineListName(displayName);
	var key = ArmyforgeUnitProfiles.smSalamanders.nameToKey[normalized];
	if (!key) {
		return null;
	}
	return ArmyforgeUnitProfiles.smSalamanders.profiles[key] || null;
};
