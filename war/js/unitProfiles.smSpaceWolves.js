...
ArmyforgeUnitProfiles.smSpaceWolves = {
	armyIds: ['SM_spacewolves_NETEA'],
	profiles: {
		...
		venerable_dreadnought:{name:'Venerable Dreadnought',type:'AV',speed:'15cm',armour:'4+',cc:'3+',ff:'4+',weapons:[{name:'Assault Cannon',range:'30cm',firepower:'AP5+/AT6+',notes:[]},{name:'Power Fist',range:'base contact',firepower:'Assault Weapons',notes:['EA(+1)','MW']}],abilities:['Walker','Leader']},
...
ArmyforgeUnitProfiles.findSmSpaceWolvesProfileByName = function(displayName, listId) {
	if (!displayName) {
		return null;
	}
	if (listId && !ArmyforgeUnitProfiles.smSpaceWolves.armyIds.member(listId)) {
		return null;
	}
	var normalized = ArmyforgeUnitProfiles.normalizeSpaceMarineListName(displayName);
	var key = ArmyforgeUnitProfiles.smSpaceWolves.nameToKey[normalized];
	if (!key) {
		return null;
	}
	return ArmyforgeUnitProfiles.smSpaceWolves.profiles[key] || null;
};
